...
func TestMinutesLeft(t *testing.T) {
	...
	if got := minutesLeft(12, 0, 1000, 2000, 0); got != 42 {
		t.Errorf("минутный запас + деньги: ждали 42, получили %d", got)
	}
	// только запас, денег нет
	if got := minutesLeft(7, 0, 0, 2000, 0); got != 7 {
		t.Errorf("только запас: ждали 7, получили %d", got)
	}
	// Е2-и3: пакет — такой же запас времени, как монеты. Гость с пятичасовым
	// пакетом и пустым кошельком видит 300 минут, а не ноль, и садится.
	if got := minutesLeft(0, 300, 0, 2000, 0); got != 300 {
		t.Errorf("только пакет: ждали 300, получили %d", got)
	}
	// Все три источника складываются: 12 монетами + 60 пакетом + 30 деньгами.
	if got := minutesLeft(12, 60, 1000, 2000, 0); got != 102 {
		t.Errorf("монеты + пакет + деньги: ждали 102, получили %d", got)
	}
}
