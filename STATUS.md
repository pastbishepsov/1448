...
- выдаются за достижения (включая первый вход), за новый уровень и с шансом за каждую
  сессию (шанс усиливает талант `case_hunter`);
...
- **применяются**: `xp_boost` (Agility) множит опыт за сессию (ур.5 = +50%);
  `case_hunter` (Strength) повышает шанс бонусного кейса за сессию;
- остальные (`cashback_master`, `luck_grade` и т.д.) — по мере появления механик
  (кэшбек при оплате, ролл тира кейса).
...
WebSocket-канал PC Shell, кейсы (+ бонус за сессию), таланты (эффекты xp_boost и
case_hunter), достижения, `PATCH /me` (логика проверена тестами/симуляцией).
