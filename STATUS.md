...
(localStorage). Режим **только показываем** (игрок вписывает сам) — ноль
античит-риска. Дальше по роадмапу: S2 — детект акселерации/мыши через shell-agent,
S3 — профиль сенсы в аккаунте (едет за игроком по клубам), S4 — полировка UX.
