...
| GET  | `/api/v1/ws/admin?token=` | live-канал админки: session/shell/deposit/booking (А6) | admin |
...
**Админка**: `web/admin.html` — редизайн-трек А0–А7 завершён 2026-07-11
(план, планировка и итоги — **ADMIN.md**, точка входа админ-сессий).
Язык «14:48 Noir», левый rail + правый drawer, иконки Phosphor duotone.
Зал — карта ПК по зонам (клик → drawer: завершение сессии arm-confirm,
ремонт); карточка гостя с формами депозит (cash/card/blik) / XP / кейс и
историей; walk-in брони с undo-возвратом; каталог в drawer с живым превью
плитки 2:3; единый журнал-аудит; owner-разделы «Статистика» и «Настройки»
(экономика в БД); live через `/ws/admin` (поллинг 7с — fallback); телефонная
вёрстка (нижняя навигация, таблицы карточками); конструктор зала (А8, owner):
позиционная схема клуба на сетке (`computers.pos_x/pos_y`,
`clubs.layout_w/layout_h` — миграция 018), редактор тап-тап — переставить,
добавить, переименовать, удалить ПК без истории, размер схемы ±.
prompt/confirm в интерфейсе нет. Форс-завершение сессии переиспользует `finishSession` — ту же логику
начисления, что и у игрока (рефакторинг `sessions.go`).
...
│   ├── migrations/          17 SQL-миграций (все таблицы)
