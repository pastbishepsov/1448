...
| POST | `/api/v1/admin/users/:id/redeem` | обмен монет гостя на время за ПК: курс считает конвертер из цены зоны (В4, миграция 028) | admin |
| POST | `/api/v1/admin/coin-redemptions/:id/void` | откат ошибочной выдачи: монеты возвращаются, строка остаётся с пометкой (В4, миграция 030) | admin |
| GET  | `/api/v1/admin/reports/coins` | монеты: обязательства клуба, эмиссия по источникам, погашения, конвертер и кэшбек | owner |
| GET/POST | `/api/v1/admin/coins/burn` | таяние монет у неактивных: предпросмотр и ручной прогон; фоновый идёт сам (В4, миграция 029) | owner |
| GET  | `/api/v1/admin/zones` | зоны зала с ценой часа (В4, миграция 027); цена часа сессии берётся из зоны ПК | admin |
| POST/PATCH/DELETE | `/api/v1/admin/zones[/:id]` | завести зону, поменять имя и цену, удалить пустую | owner |
...
| POST/GET | `/api/v1/admin/work/start` · `/work/stop` · `/work/me` | табель: отметка прихода и ухода за себя, своё состояние (В3, миграция 026) | admin |
| GET/POST/PATCH/DELETE | `/api/v1/admin/work[/:id]` | табель за период и правка записей владельцем | owner |
